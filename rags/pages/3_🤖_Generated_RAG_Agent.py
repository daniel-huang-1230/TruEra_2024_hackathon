...
# for instrumentation with TruLens evals
from trulens_eval import OpenAI, Tru, TruLlama, Select, OpenAI as fOpenAI
tru = Tru()

# if "tru" not in st.session_state.keys():
    ...
    if st.session_state.agent_messages[-1]["role"] != "assistant":
        with st.chat_message("assistant"):
            with st.spinner("Thinking..."):
                with st.session_state.tru_agent as recording:
                    
                    response = agent.chat(str(prompt), tool_choice="auto")
                    if recording not in st.session_state.keys():
                        st.session_state.tru_recording = recording
