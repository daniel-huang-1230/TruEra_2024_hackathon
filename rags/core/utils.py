...
from llama_index.core.chat_engine.types import AGENT_CHAT_RESPONSE_TYPE, StreamingAgentChatResponse, AgentChatResponse
...
from core.eval_pilot_tools import extract_eval_criteria_tool, feedback_functions_retriever_tool, custom_feedback_function_prompt_helper_tool, custom_feedback_function_instrument_tool
import chromadb
...
class RAGParams(BaseModel):
    ...
    llm: str = Field(
        default="gpt-4-0125-preview", description="LLM to use for summarization."
    )
...
def construct_agent(
    ...
    # [hackathon] add eval pilot related tools
    all_tools.append(extract_eval_criteria_tool)
    all_tools.append(feedback_functions_retriever_tool)
    all_tools.append(custom_feedback_function_prompt_helper_tool)
    all_tools.append(custom_feedback_function_instrument_tool)
    # then we add tools
